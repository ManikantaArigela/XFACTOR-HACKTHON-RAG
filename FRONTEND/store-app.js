(() => {
  ...
  function setupChatWidget() {
    ...
    document.body.insertAdjacentHTML("beforeend", `<aside class="chat-widget" data-chat-widget><button class="chat-launcher" type="button" aria-label="Open Arudhra AI assistant" aria-expanded="false" data-chat-toggle><span aria-hidden="true">✦</span><span>Ask Arudhra AI</span></button><section class="chat-panel" data-chat-panel hidden aria-label="Arudhra AI assistant"><header><div><strong>Arudhra AI</strong><small>Online shopping assistant</small></div><button class="chat-close" type="button" aria-label="Close chat" data-chat-toggle>×</button></header><div class="chat-messages" data-chat-messages><div class="chat-message bot">Hi! I can help you find products, check prices, track orders, or answer delivery questions.</div></div><form class="chat-form" data-chat-form><input name="message" placeholder="Ask about products or orders..." autocomplete="off" aria-label="Message Arudhra AI"><button type="submit" aria-label="Send message">➤</button></form></section></aside>`);
    const panel = document.querySelector("[data-chat-panel]"); const launcher = document.querySelector("[data-chat-toggle]"); const messages = document.querySelector("[data-chat-messages]");
    document.querySelectorAll("[data-chat-toggle]").forEach((button) => button.addEventListener("click", () => { const open = panel.hidden; panel.hidden = !open; launcher.setAttribute("aria-expanded", String(open)); if (open) panel.querySelector("input").focus(); }));
    document.querySelector("[data-chat-form]").addEventListener("submit", (event) => { event.preventDefault(); const input = event.currentTarget.elements.message; const text = input.value.trim(); if (!text) return; messages.insertAdjacentHTML("beforeend", `<div class="chat-message user">${escapeHtml(text)}</div>`); input.value = ""; const reply = chatReply(text); window.setTimeout(() => messages.insertAdjacentHTML("beforeend", `<div class="chat-message bot">${reply}</div>`), 180); messages.scrollTop = messages.scrollHeight; });
  }
